...
import (
	...
	"github.com/aximchain/go-sdk/types"
...
func TestTransProcess(t *testing.T) {
	...
	//----- time relock ---------
	relockResult, err := client.TimeReLock(lockResult.LockId, "test lock", ctypes.Coins{{"AXC", 200000000}}, int64(time2.Now().Add(70*time2.Second).Unix()), true)
	assert.NoError(t, err)
	fmt.Printf("timelock %d \n", relockResult.LockId)
